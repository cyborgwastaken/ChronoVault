...
import { hashPin, verifyBiometric } from '../lib/biometric';
import BiometricCapture from '../components/BiometricCapture';
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Input } from '@/components/ui/input';
import { toast } from 'sonner';
import { ShieldAlert, Download, Clock, GlobeLock, Database, Trash2, Key, FileText, CheckCircle2, ChevronUp, AlertTriangle, Fingerprint } from 'lucide-react';
...
export default function Retrieve() {
    ...
    const [biometricSimilarity, setBiometricSimilarity] = useState(null);
            ...
            if (profile?.id) {
                ...
                (dbVaults || []).forEach(db => {
                    ...
                    allVaults.push({
                        ...
                        biometric_enabled: !!db.biometric_enabled,
                        biometric_hash: db.biometric_hash,
                        biometric_embedding: db.biometric_embedding,
                        biometric_pin_hash: db.biometric_pin_hash,
                        biometric_model: db.biometric_model,
    ...
    const parseStoredEmbedding = (stored) => {
        if (!stored) return [];
    ...
    const handleBiometricVerify = async (vault) => {
        ...
        if (!vaultPinInput) {
            toast.error('PIN is required for biometric vault unlock.');
            return;
        }
        if (!biometricImage) {
            toast.error('Capture a webcam face sample first.');
            return;
        }

        setIsVerifyingBiometric(true);
        try {
            const enteredPinHash = await hashPin(vaultPinInput);
            if (enteredPinHash !== vault.biometric_pin_hash) {
                throw new Error('Invalid PIN for this vault.');
            }

            const storedEmbedding = parseStoredEmbedding(vault.biometric_embedding);
            if (!storedEmbedding.length || !vault.biometric_hash) {
                throw new Error('Stored biometric profile is incomplete.');
            }

            const result = await verifyBiometric({
                imageBase64: biometricImage,
                storedEmbedding,
                storedHash: vault.biometric_hash,
            });

            if (!result.stored_hash_integrity) {
                throw new Error('Stored biometric hash integrity check failed.');
            }

            if (!result.matched) {
                throw new Error(`Face mismatch (similarity ${(result.similarity ?? 0).toFixed(3)}).`);
            }

            setBiometricVerified(true);
            setBiometricSimilarity(result.similarity ?? null);
            toast.success('Biometric + PIN verified. Vault unlock granted.');

            if (keyFile && manifestFile) {
                await handleRebuild(vault, { skipBiometricGate: true });
            }
        } catch (err) {
            setBiometricVerified(false);
            setBiometricSimilarity(null);
    ...
    const handleRebuild = async (vault, options = {}) => {
        const { skipBiometricGate = false } = options;
        ...
        if (vault.biometric_enabled && !biometricVerified && !skipBiometricGate) {
            toast.error('Complete biometric + PIN verification before reconstruction.');
            return;
        }
    ...
    const filteredVaults = vaults.filter(v =>
        ...
        filter === 'biometric' ? v.biometric_enabled :
        !v.timer_enabled && !v.geo_enabled && !v.biometric_enabled
    );
                ...
                <div className="space-y-5">
                    ...
                    <div className="flex flex-wrap gap-1.5 items-center bg-muted/30 p-1.5 rounded-lg border border-border/25">
                        {[
                            ...
                            { id: 'biometric', label: 'Biometric+PIN' }
                        ].map(f => (
                            <Button 
                                key={f.id}
                            ...
                            <Card key={vault.id} className={`overflow-hidden transition-all duration-200 ${
                                        ...
                                        <div>
                                            ...
                                            <div className="flex flex-wrap gap-1.5 items-center mt-1.5">
                                                ...
                                                {vault.biometric_enabled && (
                                                    <Badge className="bg-cyan-500/8 text-cyan-400 border-cyan-500/20">
                                ...
                                {selectedVault?.id === vault.id && (
                                    <div className="bg-muted/20 border-t border-border/25 p-4 animate-fade-in">
                                        {vault.biometric_enabled && (
                                            <div className="mb-4 rounded-lg border border-cyan-500/20 bg-cyan-500/5 p-3 space-y-3">
                                                <div className="flex items-center justify-between">
                                                    <div className="text-xs font-semibold text-cyan-300 flex items-center gap-1.5">
                                                        <Fingerprint className="h-3.5 w-3.5" /> Biometric + PIN Gate
                                                    </div>
                                                    {biometricVerified && (
                                                        <span className="text-[10px] px-2 py-0.5 rounded-full border border-emerald-500/30 text-emerald-400 bg-emerald-500/10">
                                                            VERIFIED {biometricSimilarity !== null ? `(${biometricSimilarity.toFixed(3)})` : ''}
                                                        </span>
                                                    )}
                                                </div>
                                                ...
                                                <BiometricCapture value={biometricImage} onCapture={setBiometricImage} disabled={isVerifyingBiometric || isRebuilding} />

                                                <Button
                                                    variant="secondary"
                                                    className="w-full h-9 text-xs"
                                                    onClick={() => handleBiometricVerify(vault)}
                                                    disabled={isVerifyingBiometric || !vaultPinInput || !biometricImage || isRebuilding}
                                                >
                                                    {isVerifyingBiometric ? 'Verifying biometric gate...' : 'Verify Biometric + PIN'}
                                                </Button>
                                            </div>
                                        )}
                                        ...
                                        <div className="mt-4">
                                            <Button
                                                ...
                                                disabled={isRebuilding || !keyFile || !manifestFile || (vault.biometric_enabled && !biometricVerified)}
                                            >
                                                {isRebuilding 
                                                    ? (txStatus || "Rebuilding...") 
                                                    : !keyFile || !manifestFile 
                                                        ? "Provide credentials to reconstruct" 
                                                        : vault.biometric_enabled && !biometricVerified
                                                            ? 'Verify biometric + PIN first'
