...
### 0) Supabase migration for Biometric + PIN vaults

Run [backend/supabase_biometric_migration.sql](backend/supabase_biometric_migration.sql) in the Supabase SQL editor before using biometric vaults.
...
### 2.5) Start the Python biometric service (Siamese embeddings)
...
uvicorn biometric_service:app --host 0.0.0.0 --port 8090
```

This service handles:

- `POST /enroll` to generate a face embedding + hash from webcam capture
- `POST /enroll-batch` to generate one robust embedding from 5 webcam captures
- `POST /verify` to compare a live embedding against the stored profile hash
...
Add this in `frontend/.env` if needed:

```env
VITE_BIOMETRIC_API_URL=http://localhost:8090
```

Open the dev URL Vite prints (usually `http://localhost:5173`).

## Deployment (recommended free setup)

If you want to deploy with minimal cost:

- **Frontend:** Vercel (free)
- **Go backend API (`/upload`, `/retrieve`, `/delete`):** Render or Koyeb free web service
- **Python biometric service:** separate Render/Koyeb free web service
- **Auth + DB:** Supabase (hosted)
- **Blockchain:** Sepolia (hosted network)
...
6. Add frontend env vars in Vercel:

```env
VITE_SUPABASE_URL=...
VITE_SUPABASE_ANON_KEY=...
VITE_BACKEND_URL=https://<your-go-backend>.onrender.com
VITE_BIOMETRIC_API_URL=https://<your-biometric-backend>.onrender.com
VITE_CONTRACT_ADDRESS=...
VITE_SEPOLIA_CHAIN_ID=0xaa36a7
```

### 2) Go backend service (Render/Koyeb)
...
PINATA_JWT=...
SUPABASE_URL=...
SUPABASE_JWT_SECRET=...   # needed only for HS256 legacy token support
ALLOWED_ORIGIN=https://<your-vercel-domain>
TRUSTED_PROXY=            # optional CIDR if using a trusted reverse proxy
```

### 3) Python biometric service (Render/Koyeb)

Create a second web service from the same repo, using `backend` as root.

- Build command: `pip install -r requirements-biometric.txt`
- Start command: `uvicorn biometric_service:app --host 0.0.0.0 --port $PORT`
- Runtime: Python 3.11
...
```env
ALLOWED_ORIGIN=https://<your-vercel-domain>
BIOMETRIC_THRESHOLD=0.72
```

### 4) Final wiring

After both backend services are live:

1. Put their HTTPS URLs into Vercel env vars (`VITE_BACKEND_URL`, `VITE_BIOMETRIC_API_URL`).
2. Redeploy frontend.
3. Verify in production:
	- Login works (Supabase OAuth)
	- Upload works
	- Retrieve works
	- Biometric enroll/verify works (if enabled)

### Free-tier notes

- Free services often **sleep** after inactivity (cold-start delay).
- Biometric model startup can be slow on free instances.
- If biometric service is unstable on free tier, deploy core app first and add biometric service later.
...
### Python biometric service (same `backend/.env` is fine)
